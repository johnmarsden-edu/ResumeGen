...
import { A as AstroError, U as UnknownContentCollectionError, R as RenderUndefinedEntryError, c as createComponent, r as renderTemplate, u as unescapeHTML, f as renderUniqueStylesheet, g as renderScriptElement, h as createHeadAndContent, b as renderComponent } from './astro/server_nr2lUc5z.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_D0U_Rywp.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    let entries = [];
    if (!Object.assign(__vite_import_meta_env__, { AS: process.env.AS, name: process.env.name })?.DEV && cacheEntriesByCollection.has(collection)) {
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_D-oIC2dn.mjs').then(n => n._);
...
export { DEFAULT_HASH_PROPS as D, VALID_SUPPORTED_FORMATS as V, getCollection as a, getEntries as b, DEFAULT_OUTPUT_FORMAT as c, getEntry as g, isRemotePath as i, joinPaths as j };
